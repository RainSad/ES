 ...
 *//*
@Repository
public class OracleBaseDaoImp<T> implements BaseDao<T> {
	...
	*//**
	...
	private List<T> find(int pageNo, int pageSize, Map<String, String> where, LinkedHashMap<String, String> orderby) {
		// where 与 order by 要写在select * from table 的后面，而不是where rownum<=? ) where
		// rn>=?的后面
		...
		RowMapper<T> rowMapper = BeanPropertyRowMapper.newInstance(entityClass);
		System.out.println(sql.toString() + "============" + args.toString());
